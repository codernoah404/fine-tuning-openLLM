...
class CustomDatasetMaker():
    ...
    def get_Consultation_contents(self, idx:str):
        consultation = ""
            ...
            if self.Consultation[cons]['meta']['student_idx'] == idx:
                for conv in self.Consultation[cons]['conversation']:
                    for spk in conv['utterances']:
                        if spk['speaker_idx'].split('-')[0] =='T':
                            consultation += "<|assistant|>\n"
                            consultation += spk['utterance'] + "</s>\n"
                        else:
                            consultation += "<|user|>\n"
                            consultation += spk['utterance'] + "</s>\n"
        return consultation
    ...
    def get_response(self, idx:str): # 전문가 총평에 대한건 한 학생당 한 개씩 있다. 
        for i in self.Expert:
            if self.Expert[i]['student_idx'] == idx:
                ...
                response += f"\n\n추천 진로는 {category}이며, 이유는 다음과 같다. {Expert_summ}</s>"
    ...
    def make_Data(self, idx:str):
        conv = self.get_Consultation_contents(idx) + self.get_response(idx)
        data = {
            # 'instruction':'상담 내용을 가지고 요약 및 직업 추천을 합니다.', 
